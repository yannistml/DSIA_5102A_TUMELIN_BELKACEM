...
SECRET_KEY = "86a12dd97c05eeb467064c6933f3f06d3b6133fa18c75f6889964db8f038039e"
ALGORITHM = "HS256"
...
def login(authentified: str, request: OAuth2PasswordRequestForm, db: Session):
    ...
    if user.hashed_password != str(
        hashlib.sha256(request.password.encode()).hexdigest()
    ):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Invalid password for this username",
        )

    token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    access_token = create_access_token(
        data={"sub": str(request.username)}, expires_delta=token_expires
    )
